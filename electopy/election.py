...
class election:
    ...
    def mps(self, region):
        ...
        df = (
            ...
            .apply(lambda x: x * div)
            .apply(pd.Series)
            .unstack()
            .sort_values(ascending=False)[: self.distribution[region]]
        )

        mps_dist = df.index.get_level_values(1).value_counts()
    ...
    def parlament_composition(self, text=""):
        ...
        label = electopy.display.create_parlament_labels(
            sortedparl.rename(self.parties)
        )
