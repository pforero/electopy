...
def Diferencia(Res, NRes):
    ...
    if NRes["Votos"]["Peso"] == 1:
        ...
        ax.legend(
            (p1[locP1], p2[locP1]),
            (
                "{0:s} y {1:s} Separados".format(Partido1, Partido2),
                "{0:s} + {1:s}".format(Partido1, Partido2),
            ),
        )
    else:
        ax.legend(
            ...
            (
                "{0:s} y {1:s} Separados".format(Partido1, Partido2),
                "{0:s} + {2:2.0f}% {1:s}".format(Partido1, Partido2, Peso * 100),
            ),
        )
    ...
    ax.set_title(
        "Resultado de las elecciones si el {2:2.0f}% del voto de {1:s} fuera al {0:s}".format(
            Partido1, Partido2, Peso * 100
        )
    )
    ...
    ax.text(
        ...
        60,
        u"Diferencia:\n\n"
        + "\n".join("{:10s}{:+.0f}".format(i, dif.loc[i]) for i in order.index),
        family="monospace",
    )
    ax.text(
        9,
        60,
        u"Bloque Derecha:   {:+.0f}\nBloque Izquierda: {:+.0f}".format(
            dif.loc[dif.index.isin(DCH)].sum(), dif.loc[dif.index.isin(IZQ)].sum()
        ),
        family="monospace",
    )
    for i in ind:
